...
<html>
  <head>
    <base href="<%=basePath%>">
		...
		<style type="text/css">
			@media screen and (max-width:991px){#all-archive{width:100%;height:100%;margin-top: -15px;}}/*宽度小于px时*/
			@media screen and (min-width:991px){#all-archive{width:50%;margin-top: 15px;}}
			body {
			...
			.archivemain a:link{color:black;}
			.archivemain a:visited{color:black;}
			.archivemain a:hover{color:black;}
			.archivemain a:active{color:black;}
		</style>
	</head>
	<body>
	<div class="ui aligned center aligned grid allcontent" >
		...
		<div class="" id="all-archive" style="background:#ffffff;">
	  		<ul class="archivemain" id="">
	  		</ul>
		</div>	
	</div>
				...
				$.ajax({
				  ...
				  success:function(data){
			  	  for(var i in data){//遍历json数组时，这么写p为索引，0,1 
	  				...
	  				//" " + data[i].commentsNum);
	  				FillArchivemain(data[i]);
				  }
				  }
				});
			}); 
			
			function FillArchivemain(content){ 
				//alert(content.title+"  "+content.created);
				 var divString = '<div class="ui hidden divider"></div>'+
				 				'<li><p class="ui left aligned container">' +
				 				content.created +': <span><a href="ShowPage.action?cid='+
				 				content.cid+'">'+
				 				content.title+'</a></span></p></li>';
				$(".archivemain").append(divString);
